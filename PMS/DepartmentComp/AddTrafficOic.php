...
if (!$_SESSION['loggedin']) {
    ...
    if (isset($_POST['save'])) {
        ...
        if ($_POST['action'] == "add") {
            ...
            if (mysqli_num_rows($res_u) > 0) {
                // header("Location: http://localhost/PMS/DepartmentComp/AddTrafficOic.php?msg=already");
  ...
  <strong>Error!</strong> User Already login 
                ...
                echo '<script type="text/javascript">window.location="AddTrafficOic.php?act=1";</script>';
            }
        } else
  if ($_POST['action'] == "update") {
            $officer_id = mysqli_real_escape_string($conn, $_POST['officer_id']);
            $sql = $conn->query("UPDATE  trafficoic_tbl  SET  `first_name`='" . $first_name . "',`pol_station`='" . $pol_station . "',`email`='" . $email . "',`phone_number`='" . $phone_number . "',`rank`='" . $rank . "',`pass`='" . $pass . "'  WHERE  officer_id  = '" . $officer_id . "'");
            echo '<script type="text/javascript">window.location="AddTrafficOic.php?act=2";</script>';
        }
    }
    ...
    if (isset($_GET['action']) && $_GET['action'] == "delete") {
        ...
        $conn->query("DELETE FROM `trafficoic_tbl` WHERE officer_id='" . $_GET['officer_id'] . "'");
        header("location: AddTrafficOic.php?act=3");
    }
    ...
    if (isset($_GET['action']) && $_GET['action'] == "edit") {
        ...
        $sqlEdit = $conn->query("SELECT * FROM trafficoic_tbl WHERE officer_id='" . $officer_id . "'");
        if ($sqlEdit->num_rows) {
...
<div id="page-wrapper">
    <div id="page-inner">
        <div class="row">
            <div class="col-md-12">
                <h1 class="page-head-line">Traffic OIC
                    <?php
                    echo (isset($_GET['action']) && @$_GET['action'] == "add" || @$_GET['action'] == "edit") ?
                        ' <a href="AddTrafficOic.php" class="btn btn-primary btn-sm pull-right">Back <i class="glyphicon glyphicon-arrow-right"></i></a>' : '<a href="AddTrafficOic.php?action=add" class="btn btn-primary btn-sm pull-right"><i class="glyphicon glyphicon-plus"></i> Add </a>';
                    ?>
                </h1>
                ...
                <div class="col-sm-8 col-sm-offset-2">
                    <div class="panel panel-primary">
                        <div class="panel-heading">
                            <?php echo ($action == "add") ? "Add Traffic OIC" : "Edit Traffic OIC"; ?>
                        </div>
                        <form action="AddTrafficOic.php" method="post" id="signupForm1" class="form-horizontal">
                ...
                <div class="panel-body">
                    <div class="table-sorting table-responsive">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="tSortable22">
                            ...
                            <tbody>
                                <?php
                                $sql = "select * from trafficoic_tbl ";
                                $q = $conn->query($sql);
                                $i = 1;
                                while ($r = $q->fetch_assoc()) {
                                    echo '<tr>
                                            ...
                                            <td>' . $r['pol_station'] . '</td>
											<td>
											<a href="AddTrafficOic.php?action=edit&officer_id=' . $r['officer_id'] . '" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-edit"></span></a>
											
											<a onclick="return confirm(\'Are you sure you want to delete this record\');" href="AddTrafficOic.php?action=delete&officer_id=' . $r['officer_id'] . '" class="btn btn-danger btn-xs"><span class="glyphicon glyphicon-remove"></span></a> </td>
                                        </tr>';
                                    $i++;
                                }
                                ?>
