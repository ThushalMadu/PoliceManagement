...
if (!$_SESSION['loggedin']) {
	...
	if (isset($_POST['save'])) {
		...
		$sql = "select * from depart_user where officer_id= '" . $_SESSION['username'] . "' and pass='" . $oldpassword . "'";
		$q = $conn->query($sql);
		if ($q->num_rows > 0) {

			$sql = "update depart_user set  pass = '" . $newpassword . "' WHERE officer_id = '" . $_SESSION['username'] . "'";
			$r = $conn->query($sql);
