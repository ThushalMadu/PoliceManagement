...
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.branch','s.contact','s.joindate');
	...
	/* DB table to use */
	$sTable = " driver as s,branch as b ";
	...
	{
	$condArr[] = "s.sname like '%".mysqli_real_escape_string($conn,$_GET['driver'])."%'";
...
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.branch','s.contact','s.joindate');
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
					...
					html_entity_decode('<button class="btn btn-warning btn-xs" onclick="javascript:GetFeeForm('.$aRow['id'].')"><i class="fa fa-usd "></i> Fee Detail </button>')
