...
if (isset($_POST['save'])) {
	...
	$sql = "select * from trafficoic_tbl where officer_id= '" . $_SESSION['username'] . "' and pass='" . md5($oldpassword) . "'";
	$q = $conn->query($sql);
	if ($q->num_rows > 0) {

		$sql = "update trafficoic_tbl set  pass = '" . md5($newpassword) . "' WHERE officer_id = '" . $_SESSION['username'] . "'";
		$r = $conn->query($sql);
