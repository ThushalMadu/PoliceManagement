                    ...
                    <div class="table-sorting table-responsive">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="tSortable22">
                            ...
                            <tbody>
                                ...
                                $traofficer_id = $_SESSION['username'];
                                $querytra = "SELECT `pol_station` FROM trafficoic_tbl WHERE officer_id = '" . $traofficer_id . "'";
                                $resulttra = mysqli_query($conn, $querytra);
                                if (mysqli_num_rows($resulttra) > 0) {
                                    $rowtra = mysqli_fetch_array($resulttra);
                                    $policestation = $rowtra["pol_station"];
                                } else {
                                    echo "No record found";
                                }
                                $sql = "select * from spot_fine_tbl WHERE statuspay='pending' AND pol_station = '" . $policestation . "'";
                                $q = $conn->query($sql);
                                $i = 1;
